...
[[nodiscard]] rpl::producer<CreditsAmount> StarsBalanceForDisplay(
		not_null<Main::Session*> session) {
	const auto &settings = XGramSettings::getInstance();
	return rpl::combine(
		session->credits().balanceValue(),
		settings.profilePreviewEnabledValue(),
		settings.previewStarsBalanceValue()
	) | rpl::map([](CreditsAmount real, bool preview, int64 simulated) {
		return preview ? CreditsAmount(simulated) : real;
	});
}
		...
		object_ptr<Ui::FlatLabel>(
			...
			tr::lng_credits_balance_me_count(
				...
				(isCurrency
					...
					: StarsBalanceForDisplay(&controller()->session())
				) | rpl::map(
