...
[[nodiscard]] rpl::producer<TextWithEntities> SetStatusLabel(
	...
	return tr::xgram_XGramPreferences() | rpl::map([](const QString&) {
		return tr::link(u"XGram Preferences"_q);
	});
}
...
void MainMenu::setupAccountsToggle() {
	...
	_toggleAccounts->addClickHandler([=](Qt::MouseButton button) {
		...
		// The avatar has its own profile action. The name, XGram Preferences
		// subtitle and arrow keep Telegram's standard account-list behaviour.
		toggleAccounts();
	});
}
...
void MainMenu::setupSetEmojiStatus() {
	_setEmojiStatus->overrideLinkClickHandler([=] {
		_controller->showSettings(Settings::XGramMain::Id());
	});
}
...
void MainMenu::setupMenu() {
	...
	if (!_controller->session().supportMode()) {
		...
		if (settings.showLReadToggleInDrawer()) {
			addAction(
				tr::xgram_LReadMessages(),
				{&st::xgramLReadMenuIcon}
			)->setClickedCallback([=]() mutable
			{
				auto &ghost = XGramSettings::ghost(&controller->session());
		...
		if (settings.showSReadToggleInDrawer()) {
			...
			addAction(
				tr::xgram_SReadMessages(),
				{&st::xgramSReadMenuIcon}
			)->setClickedCallback([=]
			{
				auto box = Ui::MakeConfirmBox({
					.text = tr::xgram_ReadConfirmationBoxQuestion(),
					.confirmed = callback,
					.confirmText = tr::xgram_ReadConfirmationBoxActionText()
				});
	...
	addAction(
		tr::xgram_ClearSavedDataMenuText(),
		{ &st::menuIconClear }
	)->setClickedCallback([=] {
		controller->show(Ui::MakeConfirmBox({
			.text = tr::xgram_ClearSavedDataText(),
			.confirmed = [controller](Fn<void()> &&close) {
				XGramMessages::clearAllSavedMessages(&controller->session());
				close();
			},
			.confirmText = tr::xgram_ClearDeletedMessagesActionText(),
	...
	if (settings.showGhostToggleInDrawer()) {
		auto ghostActiveChanges = XGramSettings::getInstance().useGlobalGhostModeValue()
			| rpl::map([controller = _controller](bool) {
				return XGramSettings::ghost(&controller->session()).ghostModeActiveValue();
			})
			| rpl::flatten_latest();
	...
	if (settings.showStreamerToggleInDrawer()) {
		const auto streamerModeToggle = addAction(
			tr::xgram_StreamerModeToggle(),
			{&st::xgramStreamerModeMenuIcon}
		)->toggleOn(rpl::single(XGramFeatures::StreamerMode::isEnabled()));
