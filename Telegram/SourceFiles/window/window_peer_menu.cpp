...
#include "xgram/ui/context_menu/context_menu.h"
#include "xgram/features/forward/xgram_forward.h"
...
void Filler::addToggleNoForwards() {
	...
	const auto disabledNow = user->isXGramNoForwards();
...
void Filler::fillContextMenuActions() {
	...
	addToggleArchive();
	addTogglePin();
	XGramUi::AddXGramActions(_peer, _thread, _controller, _addAction);
	...
	addClearHistory();
	XGramUi::AddDeleteOwnMessagesAction(_peer, _topic, _controller, _addAction);
	addDeleteChat();
	addLeaveChat();
	addDeleteTopic();
}
...
void Filler::fillHistoryActions() {
	...
	addCreateTopic();
	addInfo();
	XGramUi::AddJumpToBeginningAction(_peer, _thread, _controller, _addAction);
	XGramUi::AddOpenChannelAction(_peer, _controller, _addAction);
	addViewAsTopics();
	addManageChat();
	addStoryArchive();
	addSupportInfo();
...
void Filler::fillProfileActions() {
	...
	addToggleTopicClosed();
	XGramUi::AddOpenChannelAction(_peer, _controller, _addAction);
	XGramUi::AddShadowBanAction(_peer, _addAction);
	addViewDiscussion();
	addDirectMessages();
	addExportChat();
	addToggleNoForwards();
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(
	...
	state->submit = [=](Api::SendOptions options) {
		...
		auto xgramForwarding = XGramForward::isXGramForwardNeeded(items) || XGramForward::isFullXGramForwardNeeded(items.front());

		if ((!state->submit || xgramForwarding) && successCallback) {
			successCallback();
		}
	};
