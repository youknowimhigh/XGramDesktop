...
void SetupSubButtonBackground(
	...
	background->paintRequest(
		...
		p.setBrush(st::boxBg);
		p.setPen(Qt::NoPen);
		XGramUserpic::PaintShape(p, QRectF(background->rect()));
	}, background->lifetime());
...
void UserpicButton::paintUserpicFrame(Painter &p, QPoint photoPosition) {
	...
	if (_streamed
		...
		const auto xgramOverride = XGramUserpic::ShouldOverrideShape(_shape);
		if (xgramOverride) {
			XGramUserpic::ApplyFrameRounding(
				request,
				_roundingCorners,
				_ellipseMask,
				size);
...
void UserpicButton::showCustom(QImage &&image) {
	...
	if (_userpicHasImage) {
		...
		_userpic = Ui::PixmapFromImage(
			xgramOverride
			? Images::Round(std::move(small), ImageRoundRadius::XGramUserpic)
			: useForumShape()
			? Images::Round(
				std::move(small),
...
void UserpicButton::fillShape(QPainter &p, QBrush brush) const {
	...
	const auto xgramOverride = XGramUserpic::ShouldOverrideShape(_shape);
	if (xgramOverride) {
		XGramUserpic::PaintShape(p, 0, 0, size);
	} else if (useForumShape()) {
			...
			if (_showPeerUserpic) {
				...
				if (const auto full = _nonPersonalView->image(Size::Large)) {
					...
					auto image = full->original().scaled(
						...
						Qt::IgnoreAspectRatio,
						Qt::SmoothTransformation);
					const auto xgramNP = XGramUserpic::ShouldOverrideShape(_shape);
					if (xgramNP) {
						image = Images::Round(
							std::move(image),
