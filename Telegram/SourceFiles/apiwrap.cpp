...
void ApiWrap::markContentsRead(
	...
	const auto &ghost = XGramSettings::ghost(&session());
...
void ApiWrap::forwardMessages(
	...
	const auto fullXGramForward = XGramForward::isFullXGramForwardNeeded(draft.items.front());
	if (fullXGramForward) {
		crl::async([=] {
			XGramForward::forwardMessages(_session, action, false, draft);
		});
		return;
	}

	const auto xgramIntelligentForwardNeeded = XGramForward::isXGramForwardNeeded(draft.items);
	if (xgramIntelligentForwardNeeded) {
		crl::async([=] {
			XGramForward::intelligentForward(_session, action, draft);
		});
		return;
	}
...
void ApiWrap::sendMessage(
	...
	if (!ephemeral
		&& !canSendTexts
		&& !XGramForward::isForwarding(peer->id)) {
		return;
...
void ApiWrap::sendBotStart(
	...
	request(MTPmessages_StartBot(
		...
		XGramWorker::markAsOnline(_session);
	}).fail([=](const MTP::Error &error) {
		if (chat) {
			const auto type = error.type();
